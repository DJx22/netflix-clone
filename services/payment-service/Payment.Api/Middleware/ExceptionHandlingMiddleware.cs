...
/// <para>
/// Expected failures (not-found, invalid state transition) are typed domain or
/// application exceptions — this middleware maps them to their correct HTTP status
/// codes. Genuinely unexpected exceptions fall through to 500, with full context
/// in the structured log but no internal detail in the response body.
/// </para>
/// </summary>
/// <remarks>
/// §13: never log payment amounts, card details, or any field from a request body
/// at any log level. Log only IDs and status codes.
/// </remarks>
public sealed class ExceptionHandlingMiddleware
{
    ...
    private async Task HandleExceptionAsync(HttpContext context, Exception exception)
    {
        var (statusCode, title, detail) = exception switch
        {
            ...
            DbUpdateConcurrencyException
                => (HttpStatusCode.PreconditionFailed, "Precondition Failed",
                    "The resource was modified concurrently. Fetch the current state and retry."),

            // 422 — request was structurally valid but violates a domain rule given current state
            InvalidPaymentOperationException e
            ...
            // 400 — domain argument guard (e.g. invalid currency, empty GUID)
            ArgumentException e
        ...
        {
            // §13: log method + path only — never log request body or payment fields.
